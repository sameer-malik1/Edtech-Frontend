...
function Footer() {
  ...
  return (
    <div className="w-11/12   mx-auto  ">
      <div className="flex gap-10 py-5  justify-evenly  ">
        {/* 1 columns */}
        ...
        {/* 4 columns */}
        {
          FooterLink2.length > 0 ? FooterLink2.map((col,index)=>(<div className="text-richblack-500 flex flex-col gap-2">
            <h2 className='text-white font-bold'>{col.title}</h2>
            {
              col.links.map(link=>(<Link to={link.link}><p>{link.title}</p></Link>))
            }
